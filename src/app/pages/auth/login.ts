...
@Component({
    selector: 'app-login',
    standalone: true,
    imports: [ButtonModule, CheckboxModule, InputTextModule, PasswordModule, FormsModule, RouterModule, RippleModule, AppFloatingConfigurator, Toast, ReactiveFormsModule],
                ...
                <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                    <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                        <div class="text-center mb-8">
                            ...
                            <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Welcome to Hire5</div>
                            <span class="text-muted-color font-medium">Sign in to continue</span>
                        </div>

                        <form [formGroup]="loginForm">
                            <label for="username" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Username</label>
                            <input pInputText id="username" type="text" placeholder="Enter your username" class="w-full md:w-[30rem] mb-8" formControlName="username" />

                            <label for="password" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Password</label>
                            <p-password id="password" formControlName="password" placeholder="Enter your password" [toggleMask]="true" styleClass="mb-4" [fluid]="true" [feedback]="false"></p-password>


                            <p-button (click)="onLogin()" label="Sign In" styleClass="w-full" ></p-button>
                        </form>
                    </div>
                </div>
            </div>
...
export class Login implements OnInit{
    ...
    onLogin(): void {
        if (this.authService.login(this.getFormValue('username'), this.getFormValue('password'))) {
            this.router.navigate(['products']);
        } else if (this.loginForm.invalid) {
            this.loginForm.markAllAsTouched();
            this.messageService.add({
                severity: 'error',
                summary: 'Please entry required fields'
            })
        } else {
            this.messageService.add({
                severity: 'error',
                summary: 'Wrong username or password',
                detail: 'Please check username or password'
            })
        }
    }
}
